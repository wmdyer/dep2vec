...
A string can be parsed by something like [UDPipe](https://github.com/ufal/udpipe) to reveal its dependency structure. The context for dependent words is generated as the list of (reverse) traversals from each of the leaf nodes to the root. Unigrams are the words themselves and bigrams each pair of adjacent words, where adjacency in this case represents dependency. Thus the contextual frames for the sentence above are
